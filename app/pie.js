...
import {Styles, Fonts, Screen, Colors} from './lib/styles';
...
export default class Pie extends Component {
  ...
  setPieData = async () => {
    this.expenseData = await Env.getTransactionPerCategory(
      this.state.period,
      Env.EXPENSE_TYPE,
    );
    this.incomeData = await Env.getTransactionPerCategory(
      this.state.period,
      Env.INCOME_TYPE,
    );

    this.expensePie = await this.convertToPieData(this.expenseData);
    this.incomePie = await this.convertToPieData(this.incomeData);

    let selectedType = this.state.tabSelectedIndex;
    if (this.expensePie.length === 0 && this.incomePie.length > 0) {
      selectedType = Env.INCOME_TYPE;
    }

    let isExpense = selectedType === 0;

    this.setState({
      transactionData: isExpense ? this.expenseData : this.incomeData,
      pieData: isExpense ? this.expensePie : this.incomePie,
      noTransaction: isExpense
    ...
    return (
      <View style={Styles.tabBox}>
        <TouchableOpacity
          ...
          onPress={async () => {
            await this.setState({tabSelectedIndex: 0});
            await this.setPieData();
          }}>
          <Text>Gastos</Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={async () => {
            await this.setState({tabSelectedIndex: 1});
            await this.setPieData();
          }}>
          <Text>Ingresos</Text>
        </TouchableOpacity>
        ...
        {this.state.pieData.map((item, index) => {
          return (
            <View key={index} style={Styles.legendItemBox}>
              ...
              <Text
                style={Styles.legendText}>{`${item.value.toString()}%`}</Text>
            </View>
          );
        })}
      </View>
                ...
                <View style={Styles.centerH}>
                  ...
                  <Text
                    style={[
                      Styles.legendText,
                      {flex: 1},
                    ]}>{`${item.percentage.toString()}%`}</Text>
                  <Text style={{fontSize: Fonts.h6}}>
                    {Env.formatCurrency(item.total)}
                  </Text>
                </View>
                <View
                  style={[
                    Styles.pieBar,
                    {width: this.getWidthBar(item.total, index)},
                  ]}></View>
              </View>
            </View>
          );
  ...
  renderEmpty() {
    return (
      <View style={Styles.center}>
        ...
        />
        <Text style={Styles.illustrationImageText}>Datos no disponibles</Text>
      </View>
    );
  }
